...
Events::on('pre_system', function () {
    ...
    Events::on('activities', function($activities) {
        ...
        foreach ($activities as $i => $activity) {
            $value = "";
            if ($i == 0) $value .= "VALUES ";
            $value .= "('".$db->escapeString($user->id)."', '".$db->escapeString($user->instance)."', '".$db->escapeString($activity["parent"])."', '".$db->escapeString($activity["item"])."', '".$db->escapeString($activity["action"])."', '".$db->escapeString($activity["section"])."', '". date("Y-m-d H:i:s", \strtotime("now")) ."')";
